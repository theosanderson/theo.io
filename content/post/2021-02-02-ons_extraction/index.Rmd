...
ggplot(values,aes(x=Ct,fill=symptoms)) + geom_density(alpha=0.3)
...
```{r}
simul_b117 = values %>% mutate(genes_detected=as.character(as.numeric(as.character(genes_detected))-1)) %>% filter(genes_detected>0)
ggplot(simul_b117,aes(x=Ct,fill=genes_detected)) +
  scale_fill_manual(values = c("red", "blue", "black")) + geom_density(alpha=0.5,position="stack")
